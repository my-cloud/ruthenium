...
func NewWallet(address string, lambda float64, validationTimestamp int64, genesisTimestamp int64) *Wallet {
	return &Wallet{address, genesisTimestamp, lambda, make(map[int][]*network.OutputResponse), validationTimestamp}
}
	...
	for _, blockHeight := range blockHeights {
		for _, utxo := range wallet.utxosByBlockHeight[blockHeight] {
			if utxo.HasIncome && !isIncomeCalculated {
				...
				blockTimestamp := wallet.genesisTimestamp + int64(blockHeight)*wallet.validationTimestamp
...
func (wallet *Wallet) calculateIncome(currentTimestamp int64, utxo *network.OutputResponse) uint64 {
	...
	var timestamp int64
	blockTimestamp := wallet.genesisTimestamp + int64(utxo.BlockHeight)*wallet.validationTimestamp
