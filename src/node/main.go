...
func main() {
	...
	port := flag.Uint64("port", environment.NewVariable("PORT").GetUint64Value(protocol.DefaultPort), "TCP port number for blockchain server")
	logLevel := flag.String("log-level", environment.NewVariable("LOG_LEVEL").GetStringValue("info"), "The log level")

	flag.Parse()
	app := protocol.NewHost(*mnemonic, *derivationPath, *password, *privateKey, uint16(*port), log.ParseLevel(*logLevel))
	app.Run()
}
