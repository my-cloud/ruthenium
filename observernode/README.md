...
## Launch
At root level (ruthenium folder), run the observer node using the command `go run observernode/main.go` with the add of some [program arguments](#program-arguments). For example:
```
go run observernode/main.go -host-ip=0.0.0.0
```
