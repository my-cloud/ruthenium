...
func Test_HandleTargetsRequest_AddInvalidTargets_AddTargetsNotCalled(t *testing.T) {
	...
	handler := NewHandler(new(validatornode.BlockchainMock), nil, synchronizerMock, new(validatornode.TransactionsPoolMock), new(clock.WatchMock), log.NewLoggerMock())
...
func Test_HandleSettingsRequest_ValidRequest_SettingsCalled(t *testing.T) {
	...
	handler := NewHandler(new(validatornode.BlockchainMock), expectedSettings, new(network.SynchronizerMock), new(validatornode.TransactionsPoolMock), new(clock.WatchMock), log.NewLoggerMock())
	req := gp2p.Data{}
...
func Test_HandleFirstBlockTimestampRequest_ValidRequest_FirstBlockTimestampCalled(t *testing.T) {
	...
	handler := NewHandler(blockchainMock, nil, new(network.SynchronizerMock), new(validatornode.TransactionsPoolMock), new(clock.WatchMock), log.NewLoggerMock())
	req := gp2p.Data{}
...
func Test_HandleTransactionRequest_AddValidTransaction_AddTransactionCalled(t *testing.T) {
	// Arrange
	waitGroup := sync.WaitGroup{}
	transactionsPoolMock := new(validatornode.TransactionsPoolMock)
	...
	handler := NewHandler(new(validatornode.BlockchainMock), nil, synchronizerMock, transactionsPoolMock, new(clock.WatchMock), log.NewLoggerMock())
	req := gp2p.Data{}
	waitGroup.Add(1)
...
func Test_HandleUtxosRequest_ValidUtxosRequest_UtxosByAddressCalled(t *testing.T) {
	...
	handler := NewHandler(blockchainMock, nil, new(network.SynchronizerMock), new(validatornode.TransactionsPoolMock), watchMock, log.NewLoggerMock())
	address := "address"
...
func Test_HandleTransactionsRequest_ValidTransactionsRequest_TransactionsCalled(t *testing.T) {
	...
	handler := NewHandler(new(validatornode.BlockchainMock), nil, new(network.SynchronizerMock), transactionsPoolMock, new(clock.WatchMock), log.NewLoggerMock())
	req := gp2p.Data{}
