...
import (
	...
	gp2p2 "github.com/my-cloud/ruthenium/src/node/network/p2p/gp2p"
...
func Test_HandleTargetsRequest_AddInvalidTargets_AddTargetsNotCalled(t *testing.T) {
	...
	handler := gp2p2.NewHandler(new(protocoltest.BlockchainMock), nil, synchronizerMock, new(protocoltest.TransactionsPoolMock), new(clocktest.WatchMock), logtest.NewLoggerMock())
...
func Test_HandleFirstBlockTimestampRequest_ValidRequest_FirstBlockTimestampCalled(t *testing.T) {
	...
	handler := gp2p2.NewHandler(blockchainMock, nil, new(networktest.SynchronizerMock), new(protocoltest.TransactionsPoolMock), new(clocktest.WatchMock), logtest.NewLoggerMock())
	req := gp2p.Data{}
...
func Test_HandleTransactionRequest_AddValidTransaction_AddTransactionCalled(t *testing.T) {
	...
	handler := gp2p2.NewHandler(new(protocoltest.BlockchainMock), nil, synchronizerMock, transactionsPoolMock, new(clocktest.WatchMock), logtest.NewLoggerMock())
	req := gp2p.Data{}
	waitGroup.Add(1)
...
func Test_HandleUtxosRequest_ValidUtxosRequest_UtxosByAddressCalled(t *testing.T) {
	...
	handler := gp2p2.NewHandler(blockchainMock, nil, new(networktest.SynchronizerMock), new(protocoltest.TransactionsPoolMock), watchMock, logtest.NewLoggerMock())
	address := "address"
...
func Test_HandleTransactionsRequest_ValidTransactionsRequest_TransactionsCalled(t *testing.T) {
	...
	handler := gp2p2.NewHandler(new(protocoltest.BlockchainMock), nil, new(networktest.SynchronizerMock), transactionsPoolMock, new(clocktest.WatchMock), logtest.NewLoggerMock())
	req := gp2p.Data{}
